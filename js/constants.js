...
export const JOYSTICK = {
  DEAD_ZONE: 10,   // 데드존(화면 px) — 미세 떨림으로 캐릭터가 움찔거리는 것 방지
  // 중심과 손가락 사이 최대 거리(화면 px). 이 거리를 넘으면 중심이 손가락을 따라 끌려온다.
  // [튜닝 이력] 중심 고정 방식은 손가락이 벗어난 거리만큼 방향 전환이 늦어져
  // (드래그 250px 기준 약 360ms) "렉 걸린다"는 피드백의 원인이 됐다.
  // 이 값이 방향 전환에 필요한 손가락 이동거리의 상한(MAX_RADIUS + DEAD_ZONE)을 결정한다.
  MAX_RADIUS: 48,
