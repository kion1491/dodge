...
export const OBSTACLE = {
  ...
  // 개체별 속도 배수 — 같은 시점에 나온 탄도 서로 다른 속도로 움직이게 한다.
  // 평균이 1이 되는 대칭 범위라 전체 난이도는 유지하고 체감 다양성만 얻는다
  SPEED_JITTER_MIN: 0.8,
  SPEED_JITTER_MAX: 1.2,

  // 위·아래에서 들어오는 탄의 진입 각도 상한 (도, 수직 기준).
  // 예전에는 0으로 고정돼 모든 낙하 탄이 같은 수직선을 그렸다
  VERTICAL_ANGLE_MAX: 40,
  // 위·아래 탄이 옆으로 빠지기 전까지 최소한 지나야 하는 필드 높이 비율.
  // 각도만 무작위로 두면 가장자리에서 나온 비스듬한 탄이 곧바로 이탈해
  // 탄 밀도만 떨어진다 — 이 값이 생성 x 범위를 좁혀 그것을 막는다
  MIN_TRAVERSAL: 0.6,
};
...
export const PHASE = {
  ...
  // 아래에서 올라오는 탄 등장 s — 캐릭터 시작 위치(하단)를 직접 위협하는 패턴이라
  // 0초부터 두지 않고, 수평(0s)과 대각선(60s) 사이 빈 구간에 배치했다.
  // 하단에 머무는 플레이를 깨는 것이 목적이므로 난이도 조절은 이 값으로 한다
  RISE_AT: 30,
...
export const JOYSTICK = {
  ...
  //
  // 한 방향으로 계속 이동 중일 때 손가락은 항상 중심에서 이 거리만큼 떨어진 곳에 있으므로,
  // 이 값이 곧 방향 전환 비용(= MAX_RADIUS + DEAD_ZONE 만큼의 손가락 이동)이다.
  // 반대로 값을 줄이면 8방향 판정이 예민해진다 — 옥탄트 경계까지의 좌우 여유가
  // tan(22.5°) × MAX_RADIUS 로 함께 줄어들기 때문.
  //
  // [튜닝 이력] 48 → 32. 측정값(엄지 떨림 10px 주입, 방향이 잘못 바뀌는 횟수/초):
  //   48: 반전 97ms, 흔들림 0회   40: 81ms, 0회   32: 79ms, 0회
  //   24: 80ms, 7.9회             16: 61ms, 38.5회
  // 24 아래로는 떨림만으로 방향이 튄다. 32가 흔들림 0을 유지하는 하한.
  MAX_RADIUS: 32,
  // 조이스틱 링 표시 반지름(화면 px) — 그리기 전용이며 조작 판정에는 관여하지 않는다.
  // MAX_RADIUS보다 큰 이유: 링이 엄지에 완전히 가려지지 않고 가장자리가 보이도록 하기 위함.
  RADIUS: 56,
};
